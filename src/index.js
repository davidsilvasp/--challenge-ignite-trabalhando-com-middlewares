...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const hasMoreThanTenTodos = user.todos.length > 9;

  if (!isPro && hasMoreThanTenTodos) {
    return response.status(403).json({ error: "Free plan" });
  }
...
function checksTodoExists(request, response, next) {
  ...
  const isTodoIdValid = validate(todoId);
